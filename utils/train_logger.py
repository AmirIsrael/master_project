...
class TrainLogger:
    ...
    def write_epoch_statistics(self, epoch: int, epoch_time: float, train_loss: float, norm: float,
        ...
        :param eval_score:
        """
        text = 'Time: {:.2e}, '.format(epoch_time)
        text += 'Gradient norm: {:.4e}, '.format(norm)
        text += 'Train loss: {:.2e}, '.format(train_loss)
        text += 'Train Score: {:.2f}, '.format(train_score)
        text += 'Val score: {:.2f}'.format(eval_score)
